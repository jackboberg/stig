...
pub fn schema_exists(config: &Runtime) -> bool {
    config.schema_file_path().is_file()
}
...
pub fn schema_has_content(config: &Runtime) -> bool {
...
pub fn schema_version(config: &Runtime) -> Result<Option<String>> {
    ...
    let stmts = Parser::parse_sql(&SQLiteDialect {}, &sql).with_context(|| {
        format!(
            ...
            config.schema_file_path().display()
        )
    })?;
...
pub fn schema_is_fresh(config: &Runtime, files: &[MigrationFile]) -> bool {
...
pub fn load_schema_sql(config: &Runtime) -> Result<Option<String>> {
...
pub fn write_schema_sql(config: &Runtime, sql: &str) -> Result<()> {
    let path = config.schema_file_path();
...
pub fn apply_schema_manifest(db: &Db, config: &Runtime) -> Result<usize> {
...
mod tests {
    ...
    fn apply_schema_manifest_rejects_explicit_transaction_control() {
        ...
        for manifest in [
            ...
            std::fs::write(config.schema_file_path(), manifest).unwrap();
