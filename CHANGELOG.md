...
- Refactored non-transactional directive parser: extracted duplicated `has_non_transactional_directive` and `strip_directive` logic into a unified `parse_directive` function in `src/migrate/directive.rs`. The original functions are now thin wrappers, eliminating ~320 lines of duplicated comment-parsing state machines.
